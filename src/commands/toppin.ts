...
export default class TopPinCommand extends Command {
    ...
    async handle(msg: Message, args: CommandArgs) {
        ...
        let [,, channelId, messageId] = messageMatch ?? []
        ...
        if (!botPerms.has('manageMessages')) { return msg.channel.createMessage('I do not have permission to manage messages in that channel') }
        if (!botPerms.has('viewChannel')) { return msg.channel.createMessage('I do not have permission to view that channel') }
